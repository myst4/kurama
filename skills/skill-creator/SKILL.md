...
name: skill-creator
description: >
  Author a new Kurama skill so the harness actually loads it: frontmatter the harness can
...
skill is a file the harness indexes, the validator lints, the suite pins, and somebody has to
...
|---|---|---|
| Knowledge one ecosystem already has | Kurama carries no language knowledge — that was `go-testing`'s removal in #125 | The project's own skills tree, listed in `standards:` |
...
The harness indexes exactly two keys out of your file — `name` and `description` — and the
description's literal `Trigger:` marker is what tells a reader when the skill fires. Nothing in
the body is indexed. **The `Trigger:` half is therefore the entire surface by which your skill
gets found**: the harness's own skill listing and the `AGENTS.md` row both carry that text and
nothing else, so a vague trigger is not a small documentation flaw, it is a skill that never loads.
...
|---|---|---|
| **A. Project Standards** | How the agent reads the `standards:` list from `openspec/config.yaml` — injected as `## Project Standards (files to read)`, or read from the config as fallback | Nothing — never describe a second resolution order |
...
reached by their trigger text through the harness's own skill index, exactly as the frontmatter
section describes.
