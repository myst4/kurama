...
install_skills() {
    ...
    # (*.sh); #89 lands lint-spec.sh there. A GLOB, deliberately, not a name
    # list: the next shipped helper travels without anybody remembering to
    # touch this loop.
    #
    ...
    if [ -d "$shared_src" ]; then
        ...
        for shared_file in "$shared_src"/*.md "$shared_src"/*.sh; do
            ...
            # non-executable helper is a broken install that looks perfectly
            # healthy in a file listing.
            case "$shared_base" in
